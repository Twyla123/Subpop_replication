...
# def df_to_dataclass_list(df: pd.DataFrame, dataclass_type: dataclass):
#     # Create list of dataclass objects by using df.itertuples
#     return [
#         dataclass_type(**row._asdict())
...
def get_config_path(path: Union[str, pathlib.Path]) -> pathlib.Path:
    ...
    return pathlib.Path(__file__).resolve().parents[3] / "configs" / path
